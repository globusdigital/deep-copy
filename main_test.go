...
func Test_generateFile(t *testing.T) {
	tests := []struct {
		...
		{name: "foo, skip map member", kind: "Foo", skip: "Map[k]", path: "./testdata", want: []byte(FooSkipMapFile)},
		{name: "alpha - with DeepCopy method", kind: "Alpha", path: "./testdata", want: []byte(AlphaPointer)},
		{name: "slicepointer, skip slice member", kind: "SlicePointer", skip: "[i]", path: "./testdata", want: []byte(SlicePointer)},
	}
