...
the type has a method `DeepCopy() [*]T`, that method will be reused.
...
flag. For example, given the following type:
...
```bash
deep-copy [--pointer-receiver] [--skip Selector1,Selector.Two] --type Type /path/to/package/containing/type
```
