...
func (g Generator) Generate(types []string, p *packages.Package) ([]byte, error) {
	objs := make([]object, len(types))
	for i, kind := range types {
		obj, err := locateType(kind, p)
		if err != nil {
			return nil, fmt.Errorf("locating type %q in %q: %v", kind, p.Name, err)
		}
...
func (g Generator) generateFile(p *packages.Package) ([]byte, error) {
